...
from utils import preprocess_context, detokenizer
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path="train", block_size=512):
        ...
        cached_features_file = os.path.join(
            directory, 'cache', args.model_name_or_path + "_cached_lm_" +
            args.output_dir.split(
                '/')[-1] + "_" + str(block_size) + "_" + filename
        )
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for text_block in self.make_text_blocks(text):
                tokenized_text_block = tokenizer.convert_tokens_to_ids(
                    tokenizer.tokenize(text_block))
                tokenized_text_block = tokenized_text_block[-block_size:]
                self.examples.append(
                    tokenizer.build_inputs_with_special_tokens(tokenized_text_block))
    ...
    def make_text_blocks(self, text):
        context = []
        current_user_utterance = []
        for line in text.split('\n'):
            match = re.match('^u([0-9]+)_.*= (.*)', line.strip())
...
def main():
    ...
    tokenizer.add_tokens(['PREDICT:', '[PARAM]', '[ACTION]'])
